...
import { humanizePointDate, humanizePointTime, toUpperCaseFirstLetter, isPointFavorite } from '../utils.js';
...
function createPointTemplate(point) {
  ...
  const eventType = toUpperCaseFirstLetter(type);
  const eventTimeFrom = humanizePointTime(dateFrom);
  const eventTimeTo = humanizePointTime(dateTo);
  const eventDuration = dateTo - dateFrom;
  const eventFavorite = isPointFavorite(isFavorite);
